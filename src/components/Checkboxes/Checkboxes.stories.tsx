...
export const WithPreCheckedValues: Story = {
  ...
  args: { ...examplesFromFixtures.find((f) => f.name === "with pre-checked values")?.options },
};

export const WithDividerAndNone: Story = {
  name: "with divider and None",
  args: { ...examplesFromFixtures.find((f) => f.name === "with divider and None")?.options },
};

export const WithDividerNoneAndConditionalItems: Story = {
  name: "with divider, None and conditional items",
  args: { ...examplesFromFixtures.find((f) => f.name === "with divider, None and conditional items")?.options },
};
...
export const WithLegendAsAPageHeading: Story = {
  ...
  args: { ...examplesFromFixtures.find((f) => f.name === "with legend as a page heading")?.options },
};
...
export const SmallWithDividerAndNone: Story = {
  ...
  args: { ...examplesFromFixtures.find((f) => f.name === "small with divider and None")?.options },
};